...
async function createUserDetail(ctx) {
    try {
        if (!ctx.user) {
        ...
        if (existingDetail) {
            (0, response_1.errorResponse)(ctx, 'User detail already exists', 400);
            return;
        }
        ...
        const processedDocuments = {
            idDocument: {
                front: (idDocumentFront && (idDocumentFront.newFilename || idDocumentFront.filename || idDocumentFront.path)) || '',
                back: (idDocumentBack && (idDocumentBack.newFilename || idDocumentBack.filename || idDocumentBack.path)) || '',
        ...
        if (panImage || panNumber) {
            processedDocuments.pan = {
                image: (panImage && (panImage.newFilename || panImage.filename || panImage.path)) || '',
                number: panNumber || ''
            };
        }
        if (selfieFile) {
            processedDocuments.selfie = (selfieFile && (selfieFile.newFilename || selfieFile.filename || selfieFile.path)) || '';
        }
        ...
        if (!processedDocuments.idDocument.front || !processedDocuments.idDocument.back) {
            (0, response_1.errorResponse)(ctx, 'Both front and back of ID document are required', 400);
            return;
        }
        if (!processedDocuments.idDocument.number) {
            (0, response_1.errorResponse)(ctx, 'ID document number is required', 400);
            return;
        }
        ...
        const newUserDetail = userDetailRepository.create({
            ...
            status: types_1.UserDetailStatus.PENDING,
        ...
        (0, response_1.errorResponse)(ctx, error.message || 'Failed to create user detail', 500);
    }
}
;
async function getUserDetail(ctx) {
    try {
        ...
        if (!userDetail) {
            (0, response_1.errorResponse)(ctx, 'User detail not found', 404);
            return;
        }
        (0, response_1.successResponse)(ctx, {
            id: userDetail.id,
        ...
        (0, response_1.errorResponse)(ctx, error.message || 'Failed to retrieve user detail', 500);
    }
}
;
async function getAllUserDetails(ctx) {
    try {
        if (!ctx.user) {
            (0, response_1.errorResponse)(ctx, 'User not authenticated', 401);
            return;
        ...
        if (ctx.user.role !== types_1.UserRole.ADMIN) {
            (0, response_1.errorResponse)(ctx, 'Unauthorized access', 403);
            return;
        }
        const userDetailRepository = database_1.default.getRepository(UserDetail_entity_1.UserDetail);
        ...
        (0, response_1.errorResponse)(ctx, error.message || 'Failed to retrieve user details', 500);
    }
}
;
async function updateUserDetail(ctx) {
    try {
        ...
        if (userDetail.status === types_1.UserDetailStatus.VERIFIED) {
            (0, response_1.errorResponse)(ctx, 'Cannot update verified user details', 400);
            return;
        }
        ...
        if (status) {
            if (status === types_1.UserDetailStatus.DRAFT) {
                userDetail.status = types_1.UserDetailStatus.DRAFT;
            }
            else if (status === types_1.UserDetailStatus.PENDING) {
                userDetail.status = types_1.UserDetailStatus.PENDING;
            ...
            if (userDetail.status === types_1.UserDetailStatus.REJECTED || userDetail.status === types_1.UserDetailStatus.CHANGES_REQUESTED || userDetail.status === types_1.UserDetailStatus.DRAFT) {
                userDetail.status = types_1.UserDetailStatus.PENDING;
                userDetail.submittedAt = new Date();
            }
        }
        ...
        (0, response_1.errorResponse)(ctx, error.message || 'Failed to update user detail', 500);
    }
}
;
async function verifyUserDetail(ctx) {
    try {
        if (!ctx.user) {
            (0, response_1.errorResponse)(ctx, 'User not authenticated', 401);
            return;
        ...
        // Check if user has admin privileges
        if (ctx.user.role !== types_1.UserRole.ADMIN && ctx.user.role !== types_1.UserRole.SUPER_ADMIN) {
            (0, response_1.errorResponse)(ctx, 'Unauthorized access. Admin privileges required', 403);
            return;
        }
        ...
        if (!userId || typeof userId !== 'string') {
            (0, response_1.errorResponse)(ctx, 'Invalid user ID', 400);
            return;
        }
        const userDetail = await userDetailRepository.findOne({
            where: { userId },
        ...
        const validStatuses = [
            types_1.UserDetailStatus.VERIFIED,
            types_1.UserDetailStatus.REJECTED,
            types_1.UserDetailStatus.CHANGES_REQUESTED,
            types_1.UserDetailStatus.PENDING
        ];
        if (!validStatuses.includes(status)) {
            (0, response_1.errorResponse)(ctx, `Invalid status. Must be one of: ${validStatuses.join(', ')}`, 400);
            return;
        }
        // Prevent changing status if already verified (unless changing to rejected or changes_requested)
        if (userDetail.status === types_1.UserDetailStatus.VERIFIED &&
            status !== types_1.UserDetailStatus.REJECTED &&
            status !== types_1.UserDetailStatus.CHANGES_REQUESTED) {
            (0, response_1.errorResponse)(ctx, 'Cannot modify verified user details. Use reject or request changes instead.', 400);
            return;
        }
        // Require rejection reason for rejection or changes requested
        if ((status === types_1.UserDetailStatus.REJECTED || status === types_1.UserDetailStatus.CHANGES_REQUESTED)) {
            if (!rejectionReason || rejectionReason.trim().length === 0) {
                (0, response_1.errorResponse)(ctx, 'Rejection reason is required when rejecting or requesting changes', 400);
                return;
            }
            if (rejectionReason.trim().length < 10) {
                (0, response_1.errorResponse)(ctx, 'Rejection reason must be at least 10 characters long', 400);
                return;
            }
        }
        // Update status
        ...
        else if (previousStatus === types_1.UserDetailStatus.VERIFIED) {
        ...
        let message = '';
        switch (status) {
            case types_1.UserDetailStatus.VERIFIED:
                message = 'User detail verified successfully';
                break;
            case types_1.UserDetailStatus.REJECTED:
                message = 'User detail rejected successfully';
                break;
            case types_1.UserDetailStatus.CHANGES_REQUESTED:
                message = 'Changes requested successfully';
                break;
            case types_1.UserDetailStatus.PENDING:
        ...
        (0, response_1.errorResponse)(ctx, error.message || 'Failed to verify user detail', 500);
    }
}
;
async function deleteUserDetail(ctx) {
    try {
        if (!ctx.user) {
            (0, response_1.errorResponse)(ctx, 'User not authenticated', 401);
            return;
        ...
        (0, response_1.successResponse)(ctx, null, 'User detail deleted successfully');
    }
    catch (error) {
        (0, response_1.errorResponse)(ctx, error.message || 'Failed to delete user detail', 500);
    }
}
;
