...
function broadcastShowdownVotes(roomId, room, votedPlayerId) {
    if (!room.game)
        return;
    const state = room.game.getState();
    const activePlayers = state.players.filter(p => p.status === sharedCardsEngine_1.PlayerStatus.Active);
...
function nextTurnTimeout(roomId, room) {
    if (room.currentTurnTimeout)
        clearTimeout(room.currentTurnTimeout);
...
function triggerShowdown(roomId, room) {
    if (!room.game || room.game.gameOver)
        return;
    if (room.currentTurnTimeout) {
        clearTimeout(room.currentTurnTimeout);
...
function startGame(roomId, room) {
    ...
    room.game = new teenPattiEngine_1.TeenPattiGame(ids, STARTING_CHIPS, BOOT_AMOUNT, MIN_BET);
